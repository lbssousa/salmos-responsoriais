...
<v>\VSup{1}</v>O() Se()nhor() é() mi()nha() luz() e() sal()va()ção;() *(:)
...
<v>\VSup{4}</v>Ao() Se()nhor() eu() pe()ço() a()pe()nas() u()ma() coi()sa,() *(:)
...
<v>\VSup{13}</v>Sei() que() a() bon()da()de() do() Se()nhor() eu() hei() de() ver() *(:)
na() ter()ra() dos() vi()ven()tes.() (::)
<v>\VSup{14}</v>Es()pe()ra() no() Se()nhor() e() tem() co()ra()gem,() *(:)
es()pe()ra() no() Se()nhor!() (::)